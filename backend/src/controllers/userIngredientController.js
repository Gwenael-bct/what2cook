...
async function deleteUserIngredient(req, res) {
  try {
    const userId = req.user?.id;
    ...
    if (!existing) {
      return res.status(200).json({ message: "Ingrédient non présent dans l'inventaire" });
    }
...
module.exports = { addUserIngredient, deleteUserIngredient };
